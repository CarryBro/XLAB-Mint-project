...
import com.example.xlab.XlabApplication
...
class UserAccountViewModel @Inject constructor(
    ...
    fun updateUser(user: com.example.xlab.data.User, imageUri: Uri?) {
        ...
        if (!areInputsValid) {
            viewModelScope.launch {
                _user.emit(Resource.Error("Проверьте правильность ввода"))
            }
            return
        }
    ...
    private fun saveUserInformationWithNewImage(user: com.example.xlab.data.User, imageUri: Uri) {
        viewModelScope.launch {
            try {
                val imageBitmap = MediaStore.Images.Media.getBitmap(
                    getApplication<XlabApplication>().contentResolver,
                    imageUri
                )
    ...
    private fun saveUserInformation(user: com.example.xlab.data.User, shouldRetrievedOldImage: Boolean) {
